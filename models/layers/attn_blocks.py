...
from lib.models.layers.attn import Attention, Attention_st, AttentionDouble
...
class CASTBlockDoubleToken(nn.Module):
    ...
    def __init__(self, dim, num_heads, mode, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.norm1 = norm_layer(dim)
        self.mode = mode
        self.attn_reshape = AttentionDouble(dim, mode, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                              proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
    ...
    def forward(self, x, mask=None):
        lens_z = 64
        lens_x = 256
        x_temp = self.drop_path(self.attn_reshape(self.norm1(x), mask))
        if self.mode == 't2t':
            x = torch.cat([x[:, :lens_z, :], x[:, lens_z:lens_z * 2, :] + x[:, -lens_z:, :]], dim=1)
        else:
            x = torch.cat([x[:, :lens_z, :], x[:, lens_z:lens_x+lens_z, :] + x[:, -lens_x:, :]], dim=1)
        x += x_temp
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
