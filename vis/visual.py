...
def vis_grid_attention(raw_data, attention_map, i, cmap='jet'):
    """
    :param img_path:图像路径
    ...
    # normed_map = ((attention_map - attention_map.min()) / (attention_map.max() - attention_map.min()) * 255).astype('uint8')
    normed_map = attention_map[0, :, 64:, 64:]
    normed_map = np.mean(normed_map, 0)
    amap = cv2.cvtColor(normed_map, cv2.COLOR_RGB2BGR)
    # draw the attention
    normed_map = cv2.resize(amap, (w, h))
    normed_map = normed_map / normed_map.max()
    normed_map = (normed_map * 255).astype('uint8')
    normed_map = cv2.applyColorMap(normed_map, cv2.COLORMAP_JET)
    normed_map = cv2.addWeighted(img, 0.6, normed_map, 1.0, 0)
    img_name = '{}_attn.jpg'.format(i)
    # plt.savefig(f'/he_zy/Chenlq/TBSI-main-base-middlemodel-8/attn/{img_name}', dpi=300)
    skimage.io.imsave('/he_zy/Chenlq/TBSI-main-base-middlemodel-8/attn/{}'.format(img_name), cv2.cvtColor(normed_map, cv2.COLOR_BGR2RGB))
