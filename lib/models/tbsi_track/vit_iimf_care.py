...
from lib.models.layers.initial_layer import SpShLayer
# from lib.models.layers.information_enhance import InformationEnhance
...
class VisionTransformerIIMF(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.middle_modality_x = nn.Parameter(torch.zeros(1, 256, embed_dim))
        self.middle_modality_z = nn.Parameter(torch.zeros(1, 64, embed_dim))
        ...
        # self.information_enhance = nn.ModuleList()
        # if self.tbsi_loc is not None and type(self.tbsi_loc) == list:
        #     for i in range(len(self.tbsi_loc)):
        #         self.information_enhance.append(
        #             InformationEnhance(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
        #                                drop=drop_rate,attn_drop=attn_drop_rate, drop_path=self.tbsi_drop_path[i],
        #                                norm_layer=norm_layer, act_layer=act_layer))

        self.sp_sh_layer = SpShLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                     drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.tbsi_drop_path[0],
                                     norm_layer=norm_layer, act_layer=act_layer)

        self.init_weights(weight_init)
        self.middle_fuse_x = nn.Sequential(nn.Linear(768 * 2, 768),
                                           nn.LayerNorm(768),
                                           nn.GELU())
        self.middle_fuse_z = nn.Sequential(nn.Linear(768 * 2, 768), 
                                           nn.LayerNorm(768),
                                           nn.GELU())
    ...
    def forward_features(self, z, x):
        ...
        # middle_token_x = self.middle_fuse_x(torch.cat([x_v, x_i], dim=2))
        # middle_token_z = self.middle_fuse_z(torch.cat([z_v, z_i], dim=2))

        z_v += self.pos_embed_z
        x_v += self.pos_embed_x
        z_i += self.pos_embed_z
        x_i += self.pos_embed_x
        # middle_token_x += self.pos_embed_x
        # middle_token_z += self.pos_embed_z
        ...
        # x_v, x_i, x_middle = self.sp_sh_layer(x_v, x_i, lens_z)
        # x_middle = combine_tokens(middle_token_z, middle_token_x, mode=self.cat_mode)
        ...
        # x_middle = self.pos_drop(x_middle)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.tbsi_loc is not None and i in self.tbsi_loc:
                x_v, x_i, x_middle = self.tbsi_layers[tbsi_index](x_v, x_i, x_middle, lens_z)
              # x_middle = self.information_enhance[tbsi_index](x_v, x_v_n, x_i, x_i_n, x_middle, lens_z)
                tbsi_index += 1
        x = torch.cat([x_v, x_i, x_middle], dim=1)
        aux_dict = {"attn": None}
