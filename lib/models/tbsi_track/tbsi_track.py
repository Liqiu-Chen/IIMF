...
class IIMFTrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, aux_loss=False, head_type="CORNER"):
        ...
        self.tbsi_fuse_search = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.GELU()
        )
        self.tbsi_fuse_middle = conv(hidden_dim * 2, hidden_dim)
    ...
    def forward_head(self, cat_feature, gt_score_map=None):
        ...
        enc_opt_v = cat_feature[:, num_template_token:num_template_token + num_search_token, :]
        enc_opt_i = cat_feature[:, num_template_token * 2 + num_search_token:num_template_token * 2 + num_search_token * 2, :]
        enc_opt_m = cat_feature[:, -num_search_token:, :]

        enc_opt = torch.cat([enc_opt_v, enc_opt_i], dim=2)
        opt_feat_1 = self.tbsi_fuse_search(enc_opt)
        opt_feat_2 = enc_opt_m
        opt_feat_1 = (opt_feat_1.unsqueeze(-1)).permute((0, 2, 1, 3)).contiguous()
        opt_feat_2 = (opt_feat_2.unsqueeze(-1)).permute((0, 2, 1, 3)).contiguous()
        bs, C, HW, Nq = opt_feat_2.size()
        HW = int(HW/2)
        opt_feat_1 = opt_feat_1.view(-1, C, self.feat_sz_s, self.feat_sz_s)
        opt_feat_2 = opt_feat_2.view(-1, C, self.feat_sz_s, self.feat_sz_s)
        # the next line is for abolotion study
        # opt_feat = opt_feat_2
        opt_feat = self.tbsi_fuse_middle(torch.cat([opt_feat_1, opt_feat_2], dim=1))
